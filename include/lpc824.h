...
#ifndef __LPC824_h_INCLUDED__
#define __LPC824_h_INCLUDED__
...
typedef struct SysConStruct {
  ...
  __IO uint32_t SysShbClkDiv; // 0x078
  uint32_t : 32;              // 0x07c
...
typedef struct GpioStruct {
  __IO uint8_t byte_pin[32];
  uint32_t reserved1[1016];

  __IO uint32_t word_pin[32];
  ...
  __IO uint32_t setmask;
  uint32_t reserved7[31];

  __I uint32_t clrmask;
  uint32_t reserved8[31];

  __I uint32_t xormask;
...
#define SYSTICK ((SystickStruct *)NVIC_BASE)

#endif // __LPC824_h_INCLUDED__
